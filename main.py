...
def train(epoch, data_loader, model, optimizer, criterion):
    ...
    for idx, (data, target) in enumerate(data_loader):
        ...
        optimizer.zero_grad()
        # run the forward and calculate the loss
        out = model.forward(data)
        loss = criterion(out, target)
        # run backward pass and optimize the model
        loss.backward()
        optimizer.step()
...
def validate(epoch, val_loader, model, criterion):
    # switch model to evaluation mode, specially for when we have droput or batchnorm.
    model.eval() # added
...
def main():
    ...
    if args.model == 'TwoLayerNet':
        ...
        model = MyModel()
        # model.load_state_dict(torch.load('./checkpoints/mymodel - Copy.pth'))
    ...
    if args.loss_type == "CE":
        ...
        criterion = FocalLoss(weight=per_cls_weights, gamma=2.0)
